...
1. Inspect the PR:
   ...
   gh pr view <P> --repo agorokh/scenariocraft \
     --json number,state,isDraft,statusCheckRollup,mergeable,reviewDecision
   ```
...
4. Check review threads with GraphQL because REST does not report resolution state:
   ...
   ```sh
   gh api graphql -f query='query($o:String!,$n:String!,$p:Int!){repository(owner:$o,name:$n){pullRequest(number:$p){reviewThreads(first:50){nodes{isResolved isOutdated path line comments(first:5){nodes{author{login} body}}}}}}}' -f o=agorokh -f n=scenariocraft -F p=<P>
   ```
...
8. After any push, if a reviewer is pending on the new head SHA, wait for the reviewer's
   ...
   Do not declare the PR resolved until one full post-push cooldown has elapsed when a
   reviewer was pending. A green check before the review lands is not a clean result.

9. Squash-merge only when required checks are green on the head SHA, no gating thread is
   unresolved, and every P1 is fixed:
